...
def preprocess_entities_relations(entity_dict, relation_dict, entities, relations):
    ...
    f = open(entity_dict, encoding="utf8")
    for line in f:
        ...
        for phase in phases:
            if phase == 'train':
                ...
                if score > best_score + eps:
                    ...
                    print('Test score  122 for best valid so far:', test_score)
                    print(type(answers))
                    writeToFile(answers, 'results_' + model_name + '_' + hops + '.txt')
                    suffix = ''
                    if freeze == True:
                        suffix = '_frozen'
                    checkpoint_path = '/content/drive/MyDrive/EmbedKGQA/checkpoints'
...
def process_text_file(text_file, split=False):
    data_file = open(text_file, encoding="utf8")
    data_array = []
